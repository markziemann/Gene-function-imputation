...
suppressPackageStartupMessages({c(library(R.utils), library(dplyr), library(tidyr), library(data.table), library(RColorBrewer), library(gplots), library(viridis))})
...
```{r echo=TRUE}
# summary show median != 0
summary_non_normalized <- summary(scerevisiae_pass_wide)

# normalize data
scerevisiae_pass_normalized <- as.data.frame(scale(scerevisiae_pass_wide))

# check that median = 0
summary_normalized <- summary(scerevisiae_pass_normalized)

# convert data frame to matrix
scerevisiae_pass_matrix_norm <- as.matrix(scerevisiae_pass_normalized)


cl <-as.dist(1-cor(t(scerevisiae_pass_matrix_norm), method="spearman"))
str(cl)
...
# create heatmap
heatmap.2(scerevisiae_pass_matrix_norm, main="S.cerevisiae PASS Samples",  Rowv=as.dendrogram(hr),
