...
tmm_agg_log <- cpm(dgeObj, log = TRUE)
# Check distributions of samples
hist(tmm_agg_s)
saveRDS(tmm_agg_s,"tmm_agg_s.rds")
saveRDS(tmm_agg_log,"tmm_agg_log.rds")
saveRDS(vsd,"vsd.rds")

# Compare transformation methods
pdf("Data/normalisations.pdf")
plot(vsd, main = "vsd")
plot(tmm_agg_s, main="TMM, Scaled")
plot(tmm_agg_log, main="TMM, log2")
plot(agg, main="Count Data")
hist(vsd)
hist(tmm_agg_s)
hist(tmm_agg_log)
hist(agg)
meanSdPlot(vsd)
meanSdPlot(tmm_agg_s)
meanSdPlot(tmm_agg_log)
meanSdPlot(agg)
dev.off()
...
hist(avcor,main="Pearson correlation to dataset average (TMM, Scaled)")
mtext(paste("mean=",signif(mean(avcor),3)),cex=1.2)
avcor<-cor(rowMeans(tmm_agg_s),tmm_agg_s,method="spearman")
hist(avcor,main="Spearman correlation to dataset average (TMM, Scaled)")
mtext(paste("mean=",signif(mean(avcor),3)),cex=1.2)

avcor<-cor(rowMeans(tmm_agg_s),vsd,method="pearson")
hist(avcor,main="Pearson correlation to dataset average (VSD)")
mtext(paste("mean=",signif(mean(avcor),3)),cex=1.2)
avcor<-cor(rowMeans(tmm_agg_s),vsd,method="spearman")
hist(avcor,main="Spearman correlation to dataset average(VSD)")
mtext(paste("mean=",signif(mean(avcor),3)),cex=1.2)
dev.off()
...
# each pair of items)
mds_vsd<-cmdscale(dist(t(vsd)))
saveRDS(mds_vsd,"mds_vsd.rds")

# INCLUDE SCREE PLOT HERE!
pdf("Data/MDS.pdf") 
#nolabel
plot(vsd, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by gene: Scaled data", cex=0.6, pch=19) 
#label
plot(vsd, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by gene: Scaled data", cex=0.6, pch=19)
text(vsd , labels=rownames(vsd) ,cex=0.8)
dev.off()

# Correlation Matrix
corr <- cor(t(vsd), method = "spearman")
lt <- corr
...
hist(lt, breaks = 30, xlab = "Gene Correlation Values", main = "Spearman Correlation of Variance Stabilised RNASeq Counts")
dev.off()
...
c2<-as.dist(1-cor(t(vsd), method="spearman"))
...
```{r echo=TRUE}
  ...
  set.seed(42)
  c<-as.dist(1-cor(t(vsd), method="spearman"))
