...
h_cluster <- function(counts,cutree_val,dist_met){
  ...
  set.seed(42)
  c<-as.dist(1-cor(t(counts), method=dist_met))
...
saveRDS(s_aggCL_sp[["mycl"]], "mycl_s_agg_sp.rds")
saveRDS(tmm_aggCL_sp[["mycl"]], "mycl_tmm_agg_sp.rds")
saveRDS(tmm_aggCL_pr[["mycl"]], "mycl_tmm_agg_pr.rds")
...
cl_data <- function(x1, x2) {
  for (i in x1){
    for (j in x2){
      ...
      saveRDS(clusters, as.character(nCL))
      saveRDS(x, as.character(nh))
    }
  }
}
