...
# Z-score scaling
s_agg <- scale(agg)
# Matrix is transposed to determine gene level correlation
tcs_agg <- cor(t(s_agg),method="spearman")
saveRDS(tcs_agg, "tcs_agg.rds")
# Database/dataset level correlation
cs_agg<-cor(s_agg,method="spearman")
...
# each pair of items)
tmds_agg<-cmdscale(dist(t(s_agg)))
mds_agg<-cmdscale(dist(s_agg))
...
#noabel
plot(tmds_agg, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by sample: Passed data", cex=0.6, pch=19)
#labeled
plot(tmds_agg, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by sample: Passed data", cex=0.6, pch=19)
text( tmds_agg , labels=rownames(tmds_agg) ,cex=0.8) 
#nolabel
plot(mds_agg, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by gene: Passed data", cex=0.6, pch=19) 
#label
plot(mds_agg, xlab="Coordinate 1", ylab="Coordinate 2", main="MDS by gene: Passed data", cex=0.6, pch=19)
text(mds_agg , labels=rownames(mds_agg) ,cex=0.8)
colfunc <- colorRampPalette(c("blue", "white", "red"))
dev.off()


# ---------
# Preliminary heatmaps with lower triangle

lt_tcs_agg <- tcs_agg
lt_tcs_agg[upper.tri(lt_tcs_agg)] <- NA

colfunc <- colorRampPalette(c("blue", "white", "red"))
png("Data/hm_scaled.png", width = 5*300, height = 5*300, res = 300, pointsize = 8)

heatmap.2(tcs_agg, dendrogram="row", scale="none", col = colfunc(25), trace="none",margins = c(6,6), cexRow=.4, main="Gene Correlation")
...
c<-as.dist(1-cor(t(s_agg), method="spearman"))
hr2 <- hclust(c , method="complete")
mycl <- cutree(hr2, h=max(hr2$height/1.78))
mycl_length <- length(unique(mycl))
mycl_length
clusterCols <- brewer.pal(length(unique(mycl)),"Paired")
myClusterSideBar <- clusterCols[mycl]
colfunc <- colorRampPalette(c("blue", "white", "red"))
write.table(mycl,file="Data/Clusters.txt",quote=F,sep="\t")


png("Data/PASS_clustered.png", width = 5*300, height = 5*300, res = 300, pointsize = 8)

heatmap.2(tcs_agg, main="Clustered Genes", Rowv=as.dendrogram(hr2), Colv=as.dendrogram(hr2), 
...
cor_s_log_t <- cor(t(logcounts),method="spearman")
saveRDS(cor_s_log_t, "cor_s_log_ts.rds")


png("Data/PASS_log.png", width = 5*300, height = 5*300, res = 300, pointsize = 8)

colfunc <- colorRampPalette(c("blue", "white", "red"))

heatmap.2(cor_s_log_t, col=colfunc(25), scale="none",trace="none",margins = c(6,6), cexRow=.4, main="Gene Correlation: log2")

dev.off()

c<-as.dist(1-cor(t(logcounts), method="spearman"))
hr2 <- hclust(c , method="complete")
mycl <- cutree(hr2, h=max(hr2$height/1.605))
mycl_length <- length(unique(mycl))
mycl_length
clusterCols <- brewer.pal(length(unique(mycl)),"Paired")
myClusterSideBar <- clusterCols[mycl]
...
png("Data/PASS_clust_log.png", width = 5*300, height = 5*300, res = 300, pointsize = 8)

heatmap.2(cor_s_log_t, main="Clustered Genes: log2", Rowv=as.dendrogram(hr2), Colv=as.dendrogram(hr2), 
 dendrogram="row", scale="none", col = colfunc(25), trace="none", 
 RowSideColors= myClusterSideBar)
...
saveRDS(mycl, "mycl_PASS_SandC.rds")
...
saveRDS(clusters, "clusters_SandC.rds")
...
png("Data/heatmaps_PASS_cl_col.png", width = 5*300, height = 5*300, res = 300, pointsize = 8)

# Create the heatmap
heatmap.2(cl_corr, main="S.cerevisiae PASS Samples",  Rowv=as.dendrogram(hr),
