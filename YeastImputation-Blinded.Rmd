...
kfold1000_10 <- cross_val(n=10, GO_annot=GO_table, clusters=cluster1000,
                       GO_list_perCl=GO_list_perCl1000, 
                       corr_clAll=corr_allClusters1000,
                       wGO_allClusters=wGO_allClusters1000_10,
                       clust_total=1000, thresh=0.1)
saveRDS(kfold1000_10, "kfold1000_10.rds")


wGO_allClusters1000_40 <- impute(cl_GOall = GOterms_perCl1000, 
                        corr_clAll = corr_allClusters1000,
                        clust_total = 1000, thresh = 0.4)
kfold1000_40 <- cross_val(n=10, GO_annot=GO_table, clusters=cluster1000,
                       GO_list_perCl=GO_list_perCl1000, 
                       corr_clAll=corr_allClusters1000,
                       wGO_allClusters=wGO_allClusters1000_40,
                       clust_total=1000, thresh=0.4)
saveRDS(kfold1000_40, "kfold1000_40.rds")
...
kfold1000_80 <- cross_val(n=10, GO_annot=GO_table, clusters=cluster1000,
                       GO_list_perCl=GO_list_perCl1000, 
                       corr_clAll=corr_allClusters1000,
                       wGO_allClusters=wGO_allClusters1000_80,
                       clust_total=1000, thresh=0.8)
saveRDS(kfold1000_80, "kfold1000_80.rds")
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken

kfold1000_90 <- cross_val(n=10, GO_annot=GO_table, clusters=cluster1000,
                       GO_list_perCl=GO_list_perCl1000, 
                       corr_clAll=corr_allClusters1000,
                       wGO_allClusters=wGO_allClusters1000_80,
                       clust_total=1000, thresh=0.9)
saveRDS(kfold1000_80, "kfold1000_90.rds")

kfold1000_100 <- cross_val(n=10, GO_annot=GO_table, clusters=cluster1000,
                       GO_list_perCl=GO_list_perCl1000, 
                       corr_clAll=corr_allClusters1000,
                       wGO_allClusters=wGO_allClusters1000_80,
                       clust_total=1000, thresh=1)
saveRDS(kfold1000_80, "kfold1000_100.rds")
...
# Get the list of all the kfold rds files
files = list.files(pattern = '^kfold.')
# Get the list of all the kfold rds files sans extension
files2 = file_path_sans_ext(list.files(pattern ='^kfold.'))
# put all the dataframes in one list
dat_list = lapply(files, function (x) readRDS(x))
# put the names for each dataframe in the list
names(dat_list) <- files2

# more simplified version of the code above but the name of the list has the extension
dat_list2 = sapply(files,function (x) readRDS(x), simplify = FALSE, USE.NAMES = TRUE)
...
summary_F1 <- sapply(dat_list, function(x){
  sapply(x, function(x) {x[length(x)]})
}, simplify = "array", USE.NAMES = TRUE)

summary_F1_list <- as.data.frame(summary_F1)
summary_F1_df <- t(do.call(rbind, lapply(summary_F1_list, data.frame, stringsAsFactors=FALSE)))


Mean.F1_Score <- colMeans(summary_F1_df)
summary_F1_df <- rbind(summary_F1_df, Mean.F1_Score)
rownames(summary_F1_df)[rownames(summary_F1_df) == "11"] <- "Mean.F1_Score"
saveRDS(summary_F1_df, "summary_F1_df.rds")
