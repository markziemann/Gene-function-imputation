...
#corr_cluster1 <- as.data.frame(corr_cluster1)
#corr_cluster1$GeneID <- rownames(corr_cluster1) 
...
# Comment: long process by using merge with key column = "GeneID"
# corr_value_blind1_df$GeneID <- rownames(corr_value_blind1_df)
# corr_blind1_GO <- corr_value_blind1_df[rownames(corr_value_blind1_df) %in% rownames(cluster1_GO_terms),]
# cluster1_GO_terms$GeneID <- rownames(cluster1_GO_terms)
# 
# corr_blind1_GO_weighted <- merge(corr_blind1_GO, cluster1_GO_terms, by = "GeneID")
# rownames(corr_blind1_GO_weighted) <- corr_blind1_GO_weighted$GeneID
# corr_blind1_GO_weighted[,1] <- c()
# corr_blind1_GO_weighted<- corr_blind1_GO_weighted[,1]*corr_blind1_GO_weighted[,2:ncol(corr_blind1_GO_weighted)]
# norm_corr_blind1_GO_weighted <- colSums(corr_blind1_GO_weighted)/nrow(corr_blind1_GO_weighted)

#---Function (weighted correlation per gene in a cluster) using modified merge where:
...
imputed_GO_terms_new <- names(which(norm_corr_blind1_GO_weighted > 0.1))

# using modified wGO2 function
hist(cluster1_wGO2$YOR391C)
...
# using modified wGO2 function
cluster1_wGO_df2 <- as.data.frame(do.call(rbind, cluster1_wGO2))

# using 0.1 as threshold
Clust1_threshold <- 0.5
...
# using modified wGO2 function
imputed_Clust1_df2 <- as.data.frame(sapply(cluster1_wGO_df2, function(x){
   as.numeric(x > Clust1_threshold)
  }))
row.names(imputed_Clust1_df2) <- row.names(cluster1_wGO_df2)


# Subtract input matrix (GO terms for imputed genes from original GO data frame) from imputed df
# Comment: matrices have to be the same dimensions for direct comparison so filtering has to be done

# Filter original GO matrix to include only the genes from the imputed df
...
# Order by rownames and only include GO terms (columns) from the imputed df
cluster1_inputMat1 <- cluster1_inputMat[order(rownames(cluster1_inputMat)),]
cluster1_inputMat1 <- cluster1_inputMat1[, colnames(cluster1_inputMat) %in% colnames(imputed_Clust1_df)]
# using modified wGO2 function
cluster1_inputMat2 <- cluster1_inputMat[order(rownames(cluster1_inputMat)),]
cluster1_inputMat2 <- cluster1_inputMat2[, colnames(cluster1_inputMat) %in% colnames(imputed_Clust1_df2)]


# Comment: the dimensions of the resultant df (imputed df) has to be "trimmed down" as well according to the dimensions of the cluster1_inputMat
...
cluster1_filtered_imputed <- cluster1_filtered_imputed[order(rownames(cluster1_filtered_imputed)),]

# using modified wGO2 function
cluster1_filtered_imputed2 <- imputed_Clust1_df2[rownames(imputed_Clust1_df2) %in% rownames(cluster1_inputMat2),]
cluster1_filtered_imputed2 <- cluster1_filtered_imputed2[order(rownames(cluster1_filtered_imputed2)),]

# Comparison between input df and resultant df
# True or False similarity table
Clust1_inutVSimputed <- ifelse(cluster1_inputMat1==cluster1_filtered_imputed, TRUE, FALSE)

Clust1_subtract <- cluster1_filtered_imputed - cluster1_inputMat1

Clust1_subtract2 <- cluster1_filtered_imputed2 - cluster1_inputMat2

# Values become negative when threshold is more than 0.5. By inspection, most of the data for genes in Cluster 1 falls between 0 to 0.5
